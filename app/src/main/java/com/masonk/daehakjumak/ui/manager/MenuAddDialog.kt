...
@Composable
fun MenuAddDialog(
    menuAddDialogViewModel: MenuAddDialogViewModel,
    onDismiss: () -> Unit
) {
    val menuAddDialogStep by menuAddDialogViewModel.menuAddDialogStep.collectAsState() // 메뉴 추가 단계
    ...
    Dialog(onDismissRequest = { onDismiss() }) {
        Card(
            ...
            when (menuAddDialogStep) {
                MenuAddEditDialogType.NAME -> { // 이름
                    MenuNameAddScreen(
                        onDismiss,
                        menuAddDialogViewModel
                    )
                }
...
// 메뉴 이름 수정
@Composable
fun MenuNameAddScreen(
    onDismiss: () -> Unit,
    menuAddDialogViewModel: MenuAddDialogViewModel
) {
    var name by remember { mutableStateOf("") } // 메뉴 이름
    ...
    Column(
        ...
        // 메뉴 이름
        Text(
            text = if (name.isEmpty()) "메뉴 이름" else name,
            style = MaterialTheme.typography.displayLarge,
            modifier = Modifier.padding(top = 34.dp),
            color = if (name.isEmpty()) LabelDisabled2 else LabelStrong
        )
        ...
        Button(
            ...
            modifier = Modifier
                .padding(16.dp)
                .fillMaxWidth()
                .clickable { // 이름 정보 저장, 다음 단계로 이동
                    menuAddDialogViewModel.addMenuName(name) // 메뉴 이름 추가/저장
                    menuAddDialogViewModel.moveDialogStepTo(MenuAddEditDialogType.PRICE) // 이름 -> 가격
                },
...
fun MenuPriceAddScreen(
    ...
    var price by remember { mutableStateOf("") } // 입력 가격 텍스트
    ...
    Column(
        ...
        // 금액
        Text(
            text = if (price.isEmpty()) "0원" else "${formatPriceIntToStr(formatPriceStrToInt(price))}원",
            style = MaterialTheme.typography.displayLarge,
            modifier = Modifier.padding(top = 34.dp),
            color = if (price.isEmpty()) LabelDisabled2 else LabelStrong
        )
        ...
        Button(
            ...
            modifier = Modifier
                .padding(16.dp)
                .fillMaxWidth()
                .clickable { // 가격 정보 저장, 다음 단계로 이동
                    menuAddDialogViewModel.addMenuPrice(formatPriceStrToInt(price)) // 메뉴 가격 추가/저장
                    menuAddDialogViewModel.moveDialogStepTo(MenuAddEditDialogType.IMAGE) // 가격 -> 사진
                },
            shape = MaterialTheme.shapes.medium,
            colors = ButtonDefaults.buttonColors(
                containerColor = PrimaryNormal,
    ...
    Column(
        ...
        Row(
            modifier = Modifier
                ...
                .padding(50.dp)
                .clickable {
                    menuAddDialogViewModel.completeMenuAdd() // 메뉴 추가 완료
                    onDismiss() // 다이얼로그 닫기
                },
            verticalAlignment = Alignment.CenterVertically
        ) {
